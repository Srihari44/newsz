...
let searcharticles = () =>{
  ...
  let searchUrl = proxyUrl + "https://newsapi.org/v2/everything?q=" + inpValue + "&page=1" + apiKey
  $('#searchModal').modal('show')
  $('.js-Search-con').empty();
  document.getElementById('searchTitle').innerText = "Search results for " + inpValue 
  $('.js-Search-con').empty();
  $.get(searchUrl, (results) => {
    let totalArticles = results.articles;
    totalArticles.forEach((article)=>{
      let truncArticlecontent = article.description.split(" ").splice(0,20).join(" ")
